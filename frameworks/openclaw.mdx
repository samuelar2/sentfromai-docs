---
title: 'OpenClaw'
description: 'Give your OpenClaw agent its own email address with the SentFromAI MCP server.'
---
...
[SentFromAI MCP server](/guides/mcp) gives it a real, dedicated email identity: an
...
Or set it up manually. You need a SentFromAI API key (`sf_live_…`) from the
[dashboard](https://console.sentfrom.ai).
...
You should see the `sentfromai` server with 21 tools — inboxes, messages, threads,
...
OpenClaw calls `create_inbox` (→ `claw@mail.sentfrom.ai`, live immediately), then
